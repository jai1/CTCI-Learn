...
import org.ctci.utils.BinaryTreeNode;
import org.ctci.utils.Graph;
import org.ctci.utils.Graph.GraphState;
import org.ctci.utils.GraphNode;
...
public class ChapterFourTreesAndGraph<T> {
	...
	private static void solution12() {
		...
		head.left = new BinaryTreeNode<Integer>(1);
		head.right = new BinaryTreeNode<Integer>(2);
		head.left.left = new BinaryTreeNode<Integer>(1);

		int numOfPaths = numOfPathsWithSum(head, 2, new HashMap<Integer, Integer>());
		checkArgument(numOfPaths == 3);

		numOfPaths = numOfPathsWithSum(head, 1, new HashMap<Integer, Integer>());
		checkArgument(numOfPaths == 3);

		numOfPaths = numOfPathsWithSum(head, 3, new HashMap<Integer, Integer>());
		checkArgument(numOfPaths == 2);
	}

	private static int numOfPathsWithSum(BinaryTreeNode<Integer> head, int target, HashMap<Integer, Integer> hashMap) {
		if (head == null)
			return 0;
		...
		for (Integer key : hashMap.keySet()) {
			newHashMap.put(key + head.data, hashMap.get(key));
		}

		newHashMap.put(head.data, newHashMap.getOrDefault(head.data, 0) + 1);
		...
		return totalPaths + numOfPathsWithSum(head.left, target, newHashMap)
				+ numOfPathsWithSum(head.right, target, newHashMap);
	}
